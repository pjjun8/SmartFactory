...
namespace test01
{
    internal class Menu
    {
        private int hue;  // hue value as a class-level variable
        private Mat image;  // Assuming image is a class-level Mat object
        private List<Rect> icons = new List<Rect>();
        public List<Rect> Icons => icons;
        public Mat Image => image;
        //public int Hue => hue;
        ...
        public int MouseMode => mouse_mode;
            ...
            List<string> iconNames = new List<string>
        {
            ...
            for (int i = 0, k = 0; i < btnRows; i++)
            {
                for (int j = 0; j < 2 && k < iconNames.Count; j++, k++)
                {
                    ...
                    Mat icon = Cv2.ImRead($"C:/Users/Admin/Desktop/OpenCv/opencv_ppt_cpp/icon/{iconNames[k]}.jpg", ImreadModes.Color);
                    if (icon.Empty()) continue; // 예외처리
        ...
        // Create a palette
        public void CreatePalette(int posY, Rect rectPalette) // pos_y : 마우스 클릭 y 좌표, rect_pale: 팔레트 영역
        {
            Mat palette = image.SubMat(rectPalette); // 팔레트 영역 참조
            float ratio1 = 180.0f / rectPalette.Height; // 팔레트 높이에 따른 hue 비율
            float ratio2 = 256.0f / rectPalette.Width; // 팔레트 너비에 따른 saturation 비율 
            float ratio3 = 256.0f / rectPalette.Height; // 팔레트 높이에 따른 intensity 비율

            hue = (int)Math.Round((posY - rectPalette.Y) * ratio1); // 색상팔레트 기본 색상
            ...
            for (int i = 0; i < palette.Rows; i++)
            {
                ...
                {
                    int saturation = (int)Math.Round(j * ratio2); // 채도 계산
                    int intensity = (int)Math.Round((palette.Rows - i - 1) * ratio3); // 명도 계산
                    palette.At<Vec3b>(i, j) = new Vec3b((byte)hue, (byte)saturation, (byte)intensity); // HSV 색상 지정
                }
            }
            Cv2.CvtColor(palette, palette, ColorConversionCodes.HSV2BGR); // HSV에서 RGB로 변환
        }
        //메소드 추가
        public void Command(int mode)
        {
            ...
            if (mode == Icon_Flag.PALETTE)
            {
                float ratio1 = 256.0f / icons[Icon_Flag.PALETTE].Width;
                float ratio2 = 256.0f / icons[Icon_Flag.PALETTE].Height;

                Point pt = pt2 - icons[Icon_Flag.PALETTE].BottomRight;
                int saturation = (int)System.Math.Round(pt.X * ratio1);
                int value = (int)System.Math.Round((icons[Icon_Flag.PALETTE].Height - pt.Y - 1) * ratio2);
                Scalar hsv = new Scalar((byte)hue, (byte)saturation, (byte)value);    //1줄 2시간 흘려보냄
                //Console.WriteLine($"h:{hsv[0]}, s:{hsv[1]}, v:{hsv[2]}");

                Mat m_color = image.SubMat(icons[Icon_Flag.COLOR]);
                m_color.SetTo(hsv);
                Cv2.CvtColor(m_color, m_color, ColorConversionCodes.HSV2BGR);
                Cv2.Rectangle(image, icons[Icon_Flag.COLOR], new Scalar(0, 0, 0), 1);

                //color = m_color.At<Vec3b>(10, 10).ToScalar();
                Vec3b vec = m_color.At<Vec3b>(10, 10);
                color = new Scalar(vec.Item0, vec.Item1, vec.Item2);
            }
            else if (mode == Icon_Flag.HUE_IDX)
            {
                CreatePalette(pt2.Y, icons[Icon_Flag.PALETTE]);
            }
        ...
        public void Draw(Mat image, Scalar color)
        {
            ...
            switch (draw_mode)
            {
                case Icon_Flag.DRAW_RECTANGLE:
                    Cv2.Rectangle(image, pt1, pt2, color, thickness);
                    break;

                case Icon_Flag.DRAW_LINE:
                    Cv2.Line(image, pt1, pt2, color, thickness);
                    break;
...
namespace test01
{
    internal class Program
    {
        //static List<Rect> icons = new List<Rect>();  // 아이콘의 사각형 영역 저장
        //static Mat image;
        static void Main(string[] args)
        {
            ...
            toolbox.CreateHueIndex(toolbox.Icons[Icon_Flag.HUE_IDX]); // 팔레트 생성
            toolbox.CreatePalette(startPale.Y, toolbox.Icons[Icon_Flag.PALETTE]); // 색상인덱스 생성
            ...
            while (true)
            {
                if (toolbox.MouseMode == 1) // 마우스 버튼 떼기
                {
                    toolbox.Draw(image, toolbox.Color);
                }
                else if (toolbox.MouseMode == 3) // 마우스 드래그
                {
                    if (toolbox.MouseMode == Icon_Flag.DRAW_BRUSH || toolbox.DrawMode == Icon_Flag.ERASE)
                    {
                        toolbox.Draw(image, toolbox.Color);
                    }
                    else
